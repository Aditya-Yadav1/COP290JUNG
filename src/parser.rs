...
pub fn parse_command(
    ...
    let re_cell_eq_cell_op_cell =
        Regex::new(r"^([A-Z]+)(\d+)=([A-Z]+)(\d+)([+\-*/])([A-Z]+)(\d+)$").unwrap();
    let re_cell_eq_int_op_cell =
        Regex::new(r"^([A-Z]+)(\d+)=(\d+)([+\-*/])([A-Z]+)(\d+)$").unwrap();
    let re_cell_eq_cell_op_int =
        Regex::new(r"^([A-Z]+)(\d+)=([A-Z]+)(\d+)([+\-*/])(\d+)$").unwrap();
    let re_cell_eq_func =
        Regex::new(r"^([A-Z]+)(\d+)=([A-Z]+)\(([A-Z]+)(\d+):([A-Z]+)(\d+)\)$").unwrap();
        ...
        *time = 0.0;
        ...
        if is_valid_cell(row, col, *total_rows, *total_cols) {
            ...
            *status = String::from("ok");
            ...
            sheet_functions::add_constraints(
                cell,
                cell1,
                cell2,
                String,
    ...
    // Cell = cell op cell
        ...
        *time = 0.0;
            ...
            && (op == '+' || op == '-' || op == '*' || op == '/')
        {
            ...
            let cell1 = CellInfo {
                row: val_row1 as i16 - 1,
                col: col1 as i16,
            };
            let cell2 = CellInfo {
                row: val_row2 as i16 - 1,
                col: col2 as i16,
            };
            sheet_functions::add_constraints(
                cell,
